...
from energize.misc.persistence import *  # pylint: disable=unused-wildcard-import,wildcard-import
...
class Test(unittest.TestCase):
    ...
    def test_save_checkpoint(self):
        ...
        os.makedirs(os.path.join(
            folder_name, f"run_{run}", STATS_FOLDER_NAME), exist_ok=True)
        ...
        test_checkpoint: Checkpoint = Checkpoint(
            run, None, None, None, 0, 0, None, None, 0, [], [], fake_parent, "json")
        ...
        expected_path: str = os.path.join(
            folder_name, f"run_{run}", "checkpoint.pkl")
    ...
    def test_build_overall_best_path(self):
        ...
        self.assertEqual(path, os.path.join(
            folder_name, f"run_{run}", OVERALL_BEST_FOLDER))
