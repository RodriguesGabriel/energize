...
import energize
from energize.misc.enums import Device, LayerType, OptimiserType
from energize.misc.phenotype_parser import Layer, Optimiser, ParsedNetwork
from energize.networks.torch import LearningParams
from energize.networks.torch.evaluators import LegacyEvaluator
from energize.networks.torch.model_builder import ModelBuilder


class Test(unittest.TestCase):

    logging.setLogRecordFactory(energize.logger_record_factory(0))
    ...
    def test_assemble_network_1(self):
        ...
        expected_model_structure = OrderedDict([
            ...
            ('fc-1', nn.Sequential(nn.Flatten(),
                                   nn.Linear(in_features=8*8*6,
                                             out_features=100, bias=True),
                                   nn.ReLU())),
            ('fc-2', nn.Sequential(nn.Linear(in_features=100,
             out_features=10, bias=True), nn.Softmax(dim=None)))
        ])
    ...
    def test_assemble_network_2(self):
        ...
        expected_model_structure = OrderedDict([
            ...
            ('pool_avg-1', nn.Sequential(nn.AvgPool2d(kernel_size=3,
             stride=2, padding=0), nn.ZeroPad2d((8, 7, 8, 7)))),
            ...
            ('conv-3', nn.Conv2d(1, 1, kernel_size=(1, 1),
             stride=(1, 1), padding=(10, 10)))
        ])
        self.assertEqual(repr(model._modules), repr(expected_model_structure))
